  ...
  for(i in 1:length(years_in_scope$years)){
    ...
    fixture = fixture_temp %>% 
      mutate(Day_of_Game = format(base::as.Date(Day_of_Game, format = "%d/%m/%Y"), "%d/%m/%Y"),
             ...
             Teams_ID = paste0(str_replace(Home_Team, " ","-"),Dash,str_replace(Away_Team, " ","-"),"/"),
...
get_PL_games = function(year){
  library(rvest);library(tidyverse);library(DBI);library(RSQLite);library(purrr);library(zoo);library(stringr)
  con = dbConnect(SQLite(), "Football_Records.sqlite")
  year = paste("SELECT * FROM Fixture_Detail WHERE Season_ID = ",year)
  ...
  game_temp[[1]] = game_temp[[1]] %>% 
    ...
    game = game[game$Was_Subbed != "Substitutes",] 
    ...
    for(j in 1:length(game_temp)){
    game_temp_df = data.frame(game_temp[j])
    names(game_temp_df) = c("Number","Player","Was_Subbed")
    game_temp[j] = game_temp_df
    game_temp[j] = game_temp[j] %>% 
      mutate(Is_Starting = 1,
             all_fixtures[i,Home_Team]
      )   
  }
